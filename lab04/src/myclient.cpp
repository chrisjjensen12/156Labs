...
void parse_config(string conf_file){
...
commandline_info get_commandline_args(int argc, char** argv){

    commandline_info info;
    ...
    if (*p) {
        ...
        info.servn = converted_port;
    }

    info.conf_file = argv[2];

    //parse config file
    parse_config(info.conf_file);

    //TODO: still need this stuff, just after we parse file not here 
...
void print_commandline_info(commandline_info commandline_info1){
    cout << "Servn: " << commandline_info1.servn << "\n";
    cout << "Conf File: " << commandline_info1.conf_file << "\n";
    cout << "MTU: " << commandline_info1.mtu << "\n";
    cout << "Winsz: " << commandline_info1.winsz << "\n";
    cout << "in file path: " << commandline_info1.in_file_path << "\n";
    cout << "out file path: " << commandline_info1.out_file_path << "\n";
    return;
}
...
void handle_timeout(list<packet_info> window, int sockfd, const sockaddr *pservaddr, socklen_t servlen, string out_file_path, int winsz){
    ...
    for (auto const &i: window) {
        ...
        std::cout << timeString << ", DATA, " << i.seq_num << ", " << basesn << ", " << nextsn << ", " << basesn+winsz << "\n";
...
void do_client_processing(int in_fd, int sockfd, const sockaddr *pservaddr, socklen_t servlen, int mtu, int winsz, string out_file_path){
    ...
    while (last_ack != 1) {
        ...
        while((nextsn < basesn+winsz) && done_reading == 0){
            ...
            if(n != 0){
                ...
                nextsn++;
                 ...
                 //log to stdout
                std::strftime(std::data(timeString), std::size(timeString), "%FT%TZ", std::gmtime(&time));
                std::cout << timeString << ", DATA, " << packet_num << ", " << basesn << ", " << nextsn << ", " << basesn+winsz << "\n";
        ...
        if(ack_n != -1){
            ...
            //log to stdout
            std::strftime(std::data(timeString), std::size(timeString), "%FT%TZ", std::gmtime(&time));
            std::cout << timeString << ", ACK, " << ack_seq_num << ", " << basesn << ", " << nextsn << ", " << basesn+winsz << "\n";
        ...
        if(ack_n < 0){
            if(errno == EINTR){
                ...
                //server timed out
                handle_timeout(window, sockfd, pservaddr, servlen, out_file_path, winsz);
            }else if(errno == EWOULDBLOCK){
                //server timed out
                handle_timeout(window, sockfd, pservaddr, servlen, out_file_path, winsz);
            }else{
...
int main(int argc, char** argv){
    ...
    commandline_info info = get_commandline_args(argc, argv);
    print_commandline_info(info);
    // print_info(info);
